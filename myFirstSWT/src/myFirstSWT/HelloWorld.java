...
public class HelloWorld {
	...
	/*
	 * Shell：表示桌面管理的窗口，包括顶层窗口（没有parent的窗口），
	 * 次级窗口或对话窗口（有parent）
	 */
	...
	public void open() {
		/*
		 * Display负责管理SWT和操作系统的交互，最重要的功能是
		 * 实现SWT的事件循环（本地部件界面调用），也提供了访问
		 * 操作系统信息的方法
		 */
...
//		disposed:销毁部件及其子部件，并进行垃圾回收
	...
	protected void createContents() {
		 ...
		 * 同时可以传入style来自定义窗口样式,SWT.TITLE只有标题没有最大化等按钮,
		 * SWT.RESIZE可以鼠标拖动改变窗口大小，使用|可以应用多个样式
		 */
		shell = new Shell();
